...
class DetectEngine(BaseAIEngine):
    ...
    async def process(self, input_text: str, options: Optional[dict] = None) -> dict:
        ...
        perplexity_score = await self._analyze_perplexity(input_text)
        burstiness_score = await self._analyze_burstiness(input_text)
        semantic_score = await self._analyze_semantic(input_text)
    ...
    async def _analyze_perplexity(self, text: str) -> float:
        words = text.split()
        if len(words) < 10:
            return 50.0

        sentence_lengths = [len(s.split()) for s in text.replace("!", ".").replace("?", ".").split(".") if s.strip()]
    ...
    async def _analyze_burstiness(self, text: str) -> float:
    ...
    async def _analyze_semantic(self, text: str) -> float:
        ai_patterns = [
        ...
        text_length_factor = len(text.split()) / 100

        semantic_score = min(pattern_count / max(text_length_factor, 1) * 20, 100)
